    ...
    //                war: '**/*.Jar'
     //           )
        ...
        sshagent(['Tomcat_User']) {
          bat 'ssh -o StrictHostKeyChecking=no ubuntu@13.233.120.253 "sudo systemctl tomcat stop && sudo rm -rf /opt/tomcat/webapps/*.jar && sudo cp "target\\*.jar" "/opt/tomcat/webapps/" && sudo systemctl tomcat start"'
          }
